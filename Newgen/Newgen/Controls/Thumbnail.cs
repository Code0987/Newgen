...
namespace Newgen.Controls
{
    public class Thumbnail : FrameworkElement, IDisposable
    {
        ...
        public void InitializeThumbnail(IntPtr source)
        {
            ...
            var cont = new Border();
            cont.Margin = new Thickness(3);
            cont.MouseEnter += (a, b) =>
            {
                cont.Background = new SolidColorBrush((Color)ColorConverter.ConvertFromString("#3290DFFF"));
            };
            cont.MouseLeave += (a, b) =>
            {
                cont.Background = new SolidColorBrush((Color)ColorConverter.ConvertFromString("#00000000"));
            };
            cont.VerticalAlignment = VerticalAlignment.Center;
            cont.HorizontalAlignment = HorizontalAlignment.Center;
        ...
        private void Thumbnail_LayoutUpdated(object sender, EventArgs e)
        {
            try
            {
                ...
                if (IntPtr.Zero != thumb)
                {
                    ...
                    MatrixTransform transform = (MatrixTransform)TransformToAncestor(target.RootVisual);
                    if (double.IsNaN(transform.Matrix.OffsetX) || double.IsNaN(transform.Matrix.OffsetY))
                    ...
                    Point b = transform.Transform(new Point(this.Width, this.Height));
